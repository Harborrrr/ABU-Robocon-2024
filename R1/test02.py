...
def zhongxin(img0):
    ...
    #红色
    #lower_white = (150, 90, 100)
    #upper_white= (175, 255, 255)
    ...
    if a != -1:
        ...
        center = (int(x),int(y))
        radius=int(radius)
        img0= cv2.circle(img0,center,radius,(25,0,255),2)#圆形拟合
        cx=int(m['m10']/m['m00'])
        cy=int(m['m01']/m['m00'])#计算中心
        epsilon = 0.001 * cv2.arcLength(cnt, True)
        approx = cv2.approxPolyDP(cnt, epsilon,True)
        cv2.drawContours(img0,[approx],-1,(0,255,0),3)
        return a,cx,cy,radius



def duizheng(x, y,radius): 
    # 相机内参矩阵和畸变系数  
    ...
    camera_points_3d = np.array([  
        ...
        (0, -7.5, 0),  
        (7.5, 0, 0),  
        (-7.5, 0, 0)], dtype="double")  
    image_points_2d = np.array([  
        (x, y + radius),  
        (x, y - radius),  
        (x + radius, y),  
        (x - radius, y)], dtype="double")
