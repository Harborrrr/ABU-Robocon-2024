...
class YoloParams:
    ...
    def __init__(self,  side):
        ...
        self.side = side
        self.anchors = [24,31, 39,47, 50,71,69,61, 70,155, 76,91,99,76, 101,116, 122,171]
    # def log_params(self):
...
# def parse_yolo_region_worker(args):
#     blob, resized_image_shape, original_im_shape, params, threshold, row, col, n = args
#     out_blob_n, out_blob_c, out_blob_h, out_blob_w = blob.shape
#     predictions = 1.0 / (1.0 + np.exp(-blob))

#     assert out_blob_w == out_blob_h, "Invalid size of output blob. It should be in NCHW layout and height should be equal to width. Current height = {}, current width = {}".format(
#     out_blob_h, out_blob_w)
...
#     confidence = object_probability
#     objects.append(
#     scale_bbox(x=x, y=y, height=height, width=width, class_id=class_id, confidence=confidence, im_h=orig_im_h,
#     im_w=orig_im_w, resized_im_h=resized_image_h, resized_im_w=resized_image_w))
#     return objects
...
#     args_list = [] 
#     for row, col, n in np.ndindex(params.side, params.side, params.num): 
#         args_list.append((blob, resized_image_shape, original_im_shape, params, threshold, row, col, n)) 
#         with Pool() as pool: 
#             results = pool.map(parse_yolo_region_worker, args_list) 
...
#            sys.exit(1)
    ...
    while cap.isOpened():
        ...
        print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",1 / (fpsend - fpsstart))
